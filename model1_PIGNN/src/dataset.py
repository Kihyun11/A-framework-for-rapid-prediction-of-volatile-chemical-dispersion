...
class ChemThetaDataset(Dataset):
    ...
    Returns PyG Data objects with:
      ...
      - u: [1, phys_dim_total] float tensor (graph-level features)
            where u = [standardized(CSV scalars || g)]
            and g = [LogP, TPSA] computed from SMILES
      - y: [1, k_theta] float tensor (standardized targets)
    ...
    def __init__(
        ...
        *,
        ...
        # Base scalar inputs from CSV
        u_csv_np = self.df[schema.scalar_cols].to_numpy(dtype=np.float32)

        # Targets
        y_np = self.df[schema.theta_cols].to_numpy(dtype=np.float32)

        # Compute graph-level descriptors g=[LogP, TPSA] from SMILES (no standardization yet)
        # We also keep graphs (optional) out of init to avoid RAM blowups.
        g_list: List[np.ndarray] = []
        for i in range(len(self.df)):
            smi = str(self.df.iloc[i][schema.smiles_col])
            # We only need g here; featurizer returns Data with .g
            g_data, _ = smiles_to_pyg(smi, use_3d=use_3d, seed=seed)
            g = g_data.g.detach().cpu().numpy().astype(np.float32)  # shape [2]
            g_list.append(g)

        g_np = np.stack(g_list, axis=0).astype(np.float32)  # [N, 2]

        # Concatenate: u_full = [CSV scalars || g(LogP, TPSA)]
        u_full_np = np.concatenate([u_csv_np, g_np], axis=1).astype(np.float32)

        # Fit or use scalers (IMPORTANT: fit on train only; pass scalers from train to val/test)
        if scaler_u is None:
            scaler_u = StandardScaler().fit(u_full_np)
        if scaler_y is None:
        ...
        self.u = scaler_u.transform(u_full_np).astype(np.float32)
    ...
    def get(self, idx: int) -> Data:
        ...
        smiles = str(row[self.schema.smiles_col])

        # Build full molecular graph (x, edge_index, edge_attr) (+ g exists but we don't need it here)
        graph, _ = smiles_to_pyg(smiles, use_3d=self.use_3d, seed=self.seed)

        # Use precomputed standardized u and y
        u = torch.tensor(self.u[idx], dtype=torch.float32).unsqueeze(0)  # [1, phys_dim_total]
        y = torch.tensor(self.y[idx], dtype=torch.float32).unsqueeze(0)  # [1, k_theta]
